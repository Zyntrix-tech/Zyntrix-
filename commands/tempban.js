    ...
    async execute(sock, msg, args = []) {
        ...
        if (!jid.endsWith("@g.us")) {
            await sock.sendMessage(jid, { 
                text: "ΓÅ▒∩╕Å This command only works in groups!" 
            }, { quoted: msg });
            return;
        }
        ...
        try {
            ...
            if (!isAdmin && !isOwner) {
                await sock.sendMessage(jid, { 
                    text: "ΓÅ▒∩╕Å Only admins can use this command!" 
                }, { quoted: msg });
                return;
            }
            ...
            if (!botAdmin) {
                await sock.sendMessage(jid, { 
                    text: "ΓÅ▒∩╕Å I need admin rights to ban users!" 
                }, { quoted: msg });
                return;
            }
            ...
            if (args.length < 2) {
                ...
                await sock.sendMessage(jid, { 
                    text: `ΓÅ▒∩╕Å *TEMPORARY BAN*\n\nUsage: !tempban @user <time> [reason]\n\nExamples:\nΓÇó !tempban @user 30m Spamming\nΓÇó !tempban @user 2h 1d Rudeness\nΓÇó !tempban @user 7d Misbehavior\n\nTime formats: m (minutes), h (hours), d (days)`,
                    contextInfo 
                }, { quoted: msg });
                return;
            }
            ...
            if (msg.message?.extendedTextMessage?.contextInfo?.quotedMessage) {
                ...
                await sock.sendMessage(jid, { 
                    text: "Γ¥î Please mention a user or reply to their message!" 
                }, { quoted: msg });
                return;
            }
            ...
            if (!timeMatch) {
                await sock.sendMessage(jid, { 
                    text: "Γ¥î Invalid time format! Use: 30m, 2h, 1d (m=minutes, h=hours, d=days)" 
                }, { quoted: msg });
                return;
            }
            ...
            await sock.sendMessage(jid, { 
                text: "Γ¥î Failed to ban user. Make sure I'm an admin!" 
            }, { quoted: msg });
        }
    }
};
