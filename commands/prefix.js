...
module.exports = {
    ...
    async execute(sock, msg, args = []) {
        ...
        if (!args.length) {
            ...
            await sock.sendMessage(from, { 
                text: `≡ƒôî *Your Personal Prefix*\n\n` +
                      ...
                      `ΓÇó !setprefix <symbol> - Set your personal prefix\n` +
                      `ΓÇó !setprefix reset - Reset to default (!)\n` +
                      `ΓÇó !setprefix view - View your current prefix\n\n` +
                      `*Examples:*\n` +
                      `ΓÇó !setprefix !\n` +
                      `ΓÇó !setprefix .\n` +
                      `ΓÇó !setprefix /\n` +
        ...
        if (action === 'view') {
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: `≡ƒôî Your current prefix: "${currentPrefix || '!'}"`,
                contextInfo 
            }, { quoted: msg });
            return;
        }
        ...
        if (action === 'reset' || action === 'default') {
            ...
            if (!currentPrefix) {
                const contextInfo = createForwardedContext();
                await sock.sendMessage(from, { 
                    text: `Γä╣∩╕Å You're already using the default prefix (!)`,
                    contextInfo 
                }, { quoted: msg });
                return;
            }
        ...
        if (newPrefix.length > 1) {
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: "Γ¥î Prefix must be a single character!\n\nExample: !setprefix !",
                contextInfo 
            }, { quoted: msg });
            return;
        }
        ...
        if (newPrefix === ' ' || newPrefix === '\n') {
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: "Γ¥î Invalid prefix! Please use a visible character like !, ., /, #, etc.",
                contextInfo 
            }, { quoted: msg });
            return;
        }
        ...
        if (setUserPrefix(sender, newPrefix)) {
            ...
            await sock.sendMessage(from, { 
                text: `Γ£à *Prefix Set!*\n\n` +
                      ...
                      `ΓÜá∩╕Å This prefix is personal to you only!`,
                contextInfo 
            }, { quoted: msg });
        } else {
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: "Γ¥î Failed to save your prefix configuration!",
                contextInfo 
            }, { quoted: msg });
        }
    }
