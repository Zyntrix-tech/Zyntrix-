    ...
    async execute(sock, msg, args = []) {
        ...
        if (!isSameUser(sender, currentOwner) && !msg.key.fromMe) {
            await sock.sendMessage(from, {
                text: formatMessage("SETOWNER", "Γ¥î Only the current owner can change the owner!")
            }, { quoted: msg });
            return;
        }
        ...
        if (number.length < 10 || number.length > 15) {
            await sock.sendMessage(from, {
                text: formatMessage("SETOWNER", "Γ¥î Invalid number! Provide a valid WhatsApp number (10-15 digits)")
            }, { quoted: msg });
            return;
        }
        ...
        await sock.sendMessage(from, { text: formatMessage("SETOWNER", `Γ£à Owner set to +${number}${nameText}`) }, { quoted: msg });
    }
};
