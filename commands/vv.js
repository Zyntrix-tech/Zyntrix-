...
module.exports = {
    ...
    async execute(sock, msg) {
        ...
        if (!quoted || !msg.message?.extendedTextMessage?.contextInfo) {
            return await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE', 'Γ¥ù Reply to a view-once image/video with !vv to reveal it.') })
        }
        ...
        try {
            ...
            if (sock.copyNForward) {
                ...
                await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE SAVED', '≡ƒ¢░∩╕Å Nexora ΓÜí anti view-once download completed. Content forwarded successfully.') })
                return
            }
            ...
            if (txt) {
                await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE SAVED', `≡ƒ¢░∩╕Å Nexora ΓÜí anti view-once download completed. Forwarded text:\n${txt}`) })
                return
            }
            ...
            for (const type of mediaTypes) {
                if (qmsg[type]) {
                    try {
                        ...
                        await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE SAVED', '≡ƒ¢░∩╕Å Nexora ΓÜí anti view-once download completed. Media forwarded.') })
                        return
                    } catch (e) {
            ...
            await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE', 'ΓÜá∩╕Å Unable to extract content from the quoted view-once message.') })
        } catch (e) {
            console.error('vv command error', e)
            await sock.sendMessage(from, { text: formatMessage('VIEW-ONCE', 'Γ¥î Error trying to save view-once content.') })
        }
    }
}
