...
module.exports = {
    ...
    async execute(sock, msg, args) {
        ...
        if (!args.length && !imageMsg && !videoMsg) {
            ...
            await sock.sendMessage(from, { 
                text: `≡ƒÿÄ *Sticker Maker*\n\n` +
                      ...
                      `*Options:*\n` +
                      `ΓÇó !sticker - Create sticker from image\n` +
                      `ΓÇó !sticker crop - Crop to square\n` +
                      `ΓÇó !sticker bg - Transparent background\n\n` +
        ...
        if (!mediaMessage && !imageUrl) {
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: "Γ¥î Please reply to an image or provide an image URL!\n\nExample: !sticker https://example.com/image.jpg", 
                contextInfo 
            }, { quoted: msg });
            return;
        }
        ...
        try {
            ...
            // Loading reaction
            await sock.sendMessage(from, { react: { text: '≡ƒöä', key: msg.key } });
            ...
            const contextInfo = createForwardedContext();
            await sock.sendMessage(from, { 
                text: "Γ¥î Failed to create sticker.\n\nPlease try with a different image.", 
                contextInfo 
            }, { quoted: msg });
        }
    }
